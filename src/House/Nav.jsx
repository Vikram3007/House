    ...
    <>
      <div
        className={`px-3 py-2 flex justify-between items-center fixed top-0 left-0 w-full transition-all duration-300 z-50 ${
          scrolled ? 'bg-white shadow-lg border-b-4 text-black' : 'bg-black/10 backdrop-blur-none text-white'
        }`}
      >
        {/* Logo */}
        ...
        >
          ...
          <nav className="flex flex-col items-center justify-center p-6 h-full">
            <ul className="flex flex-col gap-6 font-bold text-lg text-center">
              <li><Link to="/" onClick={() => setMenus(false)}>Home</Link></li>
              <li><Link to="/about" onClick={() => setMenus(false)}>About</Link></li>
              <li><Link to="/contact" onClick={() => setMenus(false)}>Contact</Link></li>
              <li className="bg-blue-800 text-white px-4 py-2 rounded">
                <Link to="/Login" onClick={() => setMenus(false)}>Login</Link>
              </li>
            </ul>
          </nav>
        </div>
